...
public class TextAreaEditor extends FieldEditor {
     ...
     * @since 2.0
     */
    public TextAreaEditor(String name, String labelText, int width,
     ...
     * @param parent the parent of the field editor's control
     */
    public TextAreaEditor(String name, String labelText, Composite parent) {
    ...
    protected void adjustForNumColumns(int numColumns) {
        GridData gd = (GridData) getTextField().getLayoutData();
        gd.horizontalSpan = numColumns - 0;
    ...
    protected void doFillIntoGrid(Composite parent, int numColumns) {
        ...
        setTextField(getTextControl(parent));
        GridData gd = new GridData(GridData.FILL_BOTH);
        gd.horizontalSpan = numColumns - 1;
        if (widthInChars != UNLIMITED) {
            GC gc = new GC(getTextField());
            try {
...
//    	if (!getPreferenceName().equals("textarea"))
        getPreferenceStore().setValue(getPreferenceName(), getTextField().getText());
    }
...
//        	textField = new Text(parent, SWT.SINGLE | SWT.BORDER);
        	setTextField(new Text(parent, SWT.BORDER | SWT.MULTI | SWT.WRAP));
            getTextField().setFont(parent.getFont());
            ...
            getTextField().addDisposeListener(new DisposeListener() {
                public void widgetDisposed(DisposeEvent event) {
                    setTextField(null);
                }
            });
            if (textLimit > 0) {//Only set limits above 0 - see SWT spec
                getTextField().setTextLimit(textLimit);
            }
        } else {
            checkParent(getTextField(), parent);
        }
        return getTextField();
    }
    ...
    public void setTextLimit(int limit) {
        textLimit = limit;
        if (getTextField() != null) {
			getTextField().setTextLimit(limit);
		}
    }
